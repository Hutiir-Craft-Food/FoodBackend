...
public class ProductImagesService {
    ...
    public ProductImagesResponse uploadImages(Long productId, ProductImagesUploadAndChanges jsonImages,
        ...
        validateNoDuplicates(product, jsonImages);
    ...
    private void validateNoDuplicates(ProductEntity product, ProductImagesUploadAndChanges jsonImage) {

        if (hasAnyDuplicatePosition(product, jsonImage)) {
            throw new BadRequestException(ProductImagesResponseMessages.ERROR_POSITION_EXIST);
        }

        if (hasExistingUid(product, jsonImage)) {
    ...
    private boolean hasExistingUid(ProductEntity product, ProductImagesUploadAndChanges jsonImage) {
        Set<String> existingUid = imageRepository.findByProductId(product.getId()).stream()
    ...
    private void validateAllImagesExistByUid(List<ImagesUploadAndChanges> requestImages, List<ProductImagesEntity> allImages) {
    ...
    private String getLink(MultipartFile file, ImageSizes size) {
        return switch (size) { //TODO заглушки для ресайза
