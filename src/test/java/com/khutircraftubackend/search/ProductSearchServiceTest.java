...
class ProductSearchServiceTest {
    ...
    class SearchProducts {
        @Test
        void shouldFallbackToSearchProductsWithLikeOnException() {
            String validQuery = "apple banana";
            ProductSearchQuery query = new ProductSearchQuery(validQuery);
            
            ProductSearchResult product1 = mock(ProductSearchResult.class);
            ProductSearchResult product2 = mock(ProductSearchResult.class);
            
            when(product1.getName()).thenReturn("Apple");
            when(product2.getName()).thenReturn("Banana");
            
            List<ProductSearchResult> mockResults = List.of(product1, product2);
            
            when(productRepository.searchProducts(validQuery)).thenThrow(new DataAccessException("Simulated database error") {
            });
            when(productRepository.searchProductsWithLike(validQuery)).thenReturn(mockResults);
            
            List<ProductSearchResult> results = productSearchService.searchProductsByQuery(query);
            
            assertNotNull(results);
            assertThat(results).hasSize(2);
            assertThat(results.get(0).getName()).isEqualTo("Apple");
            assertThat(results.get(1).getName()).isEqualTo("Banana");
            
            verify(productRepository).searchProducts(validQuery);
            verify(productRepository).searchProductsWithLike(validQuery);
        }
        @ParameterizedTest
        @ValueSource(strings = {"", "   ", "\t", "\n"})
    ...
    class SearchException {
        ...
        void shouldThrowGeneralExceptionOnCriticalError() {
            ...
            when(productRepository.searchProducts(validQuery)).thenThrow(new GeneralException("Simulated critical error"));
            ...
            verify(productRepository).searchProducts(validQuery);
            verify(productRepository, never()).searchProductsWithLike(validQuery);
        }
